...
import { TargetOccupancy } from 'views/SafetyGoal/TargetOccupancy/TargetOccupancy';
import { TypeOfSpace } from 'views/SpaceInformation/TypeOfSpace/TypeOfSpace';
import { AreOfSpace } from 'views/SpaceInformation/AreaOfSpace/AreaOfSpace';
import { CeilingHeight } from 'views/SpaceInformation/CeilingHeight/CeilingHeight';
import { OccupantAgeGroup } from 'views/UsageAndBehavior/OccupantAgeGroups/OccupantAgeGroups';
import { PhysicalActivity } from 'views/UsageAndBehavior/PhysicalActivity/PhysicalActivity';
import { VocalActivity } from 'views/UsageAndBehavior/VocalActivity/VocalActivity';
import { TypeOfMask } from './views/MaskUsage/TypeOfMask/TypeOfMask';
import { MaskFit } from './views/MaskUsage/MaskFit/MaskFit';
import { Ventilation_Filtration } from 'views/Ventilation/Ventilation_Filtration';
import { Location } from 'views/SpaceInformation/Location/Location';
...
export const PageManagement: React.FC<{}> = (props) => {
  ...
  return (
    <div className="m-16 h-full">
      ...
      {contextState.page[contextState.index] === 'Location' && (
        <Location {...LocationDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Type of Space' && (
        <TypeOfSpace {...TypeOfSpaceDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Area of Space' && (
        <AreOfSpace {...AreaOfSpaceDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Ceiling Height' && (
        <CeilingHeight {...CeilingHeightDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Occupant Age Groups' && (
        <OccupantAgeGroup {...OccupantAgeGroupDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Vocal Activity' && (
        <VocalActivity {...VocalActivityDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Physical Activity' && (
        <PhysicalActivity {...PhysicalActivityDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Types of Masks' && (
        <TypeOfMask {...TypeOfMaskDefaultProps} />
      )}
      {contextState.page[contextState.index] === 'Mask Fit' && (
        <MaskFit {...MaskFitDefaultProps} />
      )}
